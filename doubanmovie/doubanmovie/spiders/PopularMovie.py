...
from doubanmovie.items import PopularMovieItem
...
'''
class PopularMovieSpider(scrapy.Spider):
	name = 'PopularMovie'
	start_urls = ['https://movie.douban.com/j/search_subjects?type=movie&tag=%E7%83%AD%E9%97%A8&sort=recommend&page_limit=1000&page_start=0']
	headers = {'User-Agent': '''Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36'''}

	custom_settings = {
		'ITEM_PIPELINES': {
			'doubanmovie.pipelines.PopularMovieSQLPipeline': 300,
		}
	}

	def __init__(self, *a, **kw):
		super(PopularMovieSpider, self).__init__(self.name, **kw)
		self.turn = a[0]
